...
export function updateSelection(
    ...
    try {
        ...
        // GUARD 1: Check if bounds tree exists
        if (!modelObj.mesh.geometry.boundsTree) {
            console.error('Bounds tree is undefined');
        ...
        // GUARD 2: Check if any triangles were selected
        if (indices.length === 0) {
            console.warn("updateSelection: No triangles selected (empty selection)");
            return { all: null, foam: null, sense: null };
        }
        ...
        // GUARD 3: Verify the created selected mesh is valid before proceeding
        const selectedMesh = visualizer.current_selection_type === 'foam'
            ? modelObj.selectedRegularFoamMesh
            : modelObj.selectedSenseFoamMesh;

        if (!validateGeometryWithLogging(selectedMesh, 'updateSelection - created selected mesh')) {
            console.warn("Created selected mesh has invalid geometry — aborting further processing");
            return { all: null, foam: null, sense: null };
        }

        visualizer.currentSelectedModel = modelObj;

        console.log("updated selected mesh bounding box call from updateSelection");
        ...
        console.error("Critical error in updateSelection:", error);
        // Return null toolpaths on error — prevents downstream crashes
        return { all: null, foam: null, sense: null };
    }
}